...
- `../Sources`.
  The majority of Airlock source code. Managed by SPM `../Package.swift`
- `../Sources/AppBundle/`.
  Airlock.app server. Technically, it's a SPM library that is exposed to `xcode-app-bundle-launcher`
- `../Sources/Cli/`.
...
- `../xcode-app-bundle-launcher/`.
  ...
  The Xcode project model is located in `../Airlock.xcodeproj/` and generated by `../project.yml` "skeleton"
...
`airlock` CLI binary is client. `Airlock.app` is server. Client and server talk to each other via predefined UNIX file.
...
Command checklist:
- [ ] Documentation in `../docs/airlock-*` and `../docs/commands.adoc`
