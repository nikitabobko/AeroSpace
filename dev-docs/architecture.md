...
- `../Sources`.
  The majority of Aeroshift source code. Managed by SPM `../Package.swift`
- `../Sources/AppBundle/`.
  Aeroshift.app server. Technically, it's a SPM library that is exposed to `xcode-app-bundle-launcher`
- `../Sources/Cli/`.
...
- `../xcode-app-bundle-launcher/`.
  ...
  The Xcode project model is located in `../Aeroshift.xcodeproj/` and generated by `../project.yml` "skeleton"
...
`aeroshift` CLI binary is client. `Aeroshift.app` is server. Client and server talk to each other via predefined UNIX file.
...
Command checklist:
- [ ] Documentation in `../docs/aeroshift-*` and `../docs/commands.adoc`
