...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func updateStatusBarIcon() {
        ...
        let xOffset = max(0, (buttonFrame.width - iconView.frame.width) / 2)
        let yOffset = max(0, (buttonFrame.height - iconView.frame.height) / 2)
        iconView.frame.origin = CGPoint(x: xOffset, y: yOffset)
    ...
    private func updateMenuItems() {
        ...
        let copyIdToClip = NSMenuItem(title: "Copy to clipboard", action: #selector(copyIdToClip(_:)), keyEquivalent: "c")
        ...
        if viewModel.isEnabled {
            ...
            for workspace in Workspace.all {
                ...
                let workspaceItem = NSMenuItem(title: workspace.name + monitor, action: #selector(workspaceSelected(_:)), keyEquivalent: "")
        ...
        let enableItem = NSMenuItem(title: viewModel.isEnabled ? "Disable" : "Enable", action: #selector(toggleEnabled(_:)), keyEquivalent: "e")
        enableItem.target = self
        menu.addItem(enableItem)
        
        // Add open config item
        let editor = getTextEditorToOpenConfig()
        let openConfigItem = NSMenuItem(title: "Open config in '\(editor.lastPathComponent)'", action: #selector(openConfig(_:)), keyEquivalent: "o")
        openConfigItem.target = self
        menu.addItem(openConfigItem)
        
        // Add reload config item if enabled
        if viewModel.isEnabled {
            let reloadConfigItem = NSMenuItem(title: "Reload config", action: #selector(reloadConfigAction(_:)), keyEquivalent: "r")
