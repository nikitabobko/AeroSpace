...
final class WorkspaceCommandTest: XCTestCase {
    ...
    func testParseWorkspaceCommand() {
        ...
        testParseCommandSucc("workspace next", WorkspaceCmdArgs(.relative(WTarget.Relative(isNext: true, wrapAround: false))))
        testParseCommandSucc("workspace --auto-back-and-forth W", WorkspaceCmdArgs(.direct(WTarget.Direct("W", autoBackAndForth: true))))
        ...
        testParseCommandSucc("workspace next --wrap-around", WorkspaceCmdArgs(.relative(WTarget.Relative(isNext: true, wrapAround: true))))
    }
}
