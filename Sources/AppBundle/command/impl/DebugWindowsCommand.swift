...
private func dumpWindowDebugInfo(_ window: Window) async throws -> String {
    ...
    let appId = window.app.id ?? "NULL-APP-BUNDLE-ID"
    ...
    // result.append("\(windowPrefix) isWindow: \(isWindow(window.axWindow, app))") // todo
    // result.append("\(windowPrefix) isDialogHeuristic: \(isDialogHeuristic(window.axWindow, app))") // todo
    result.append(try await window.dumpAx(windowPrefix))

    let appPrefix = appId.padding(toLength: windowPrefix.count, withPad: " ", startingAt: 0)
    result.append(try await window.macApp.dumpAppAx(appPrefix))
