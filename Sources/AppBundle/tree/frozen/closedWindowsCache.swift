...
/// becomes nil, etc.) which tricks Aeroshift into thinking that all windows were closed.
/// That's why every time a window dies Aeroshift caches the "entire world" (unless window is already presented in the cache)
/// so that once the screen is unlocked, Aeroshift could restore windows to where they were
