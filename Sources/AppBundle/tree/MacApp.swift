...
final class MacApp: AbstractApp {
    ...
    private static func resolvedProcessIdentifier(for nsApp: NSRunningApplication) -> pid_t? {
        ...
        guard let executableURL = nsApp.executableURL else {
            return nil
        }
        ...
        for window in windows {
            ...
            let candidatePid = pid_t(ownerPid)

            guard candidatePid > 0 else {
                continue
            }

            guard testedPids.insert(candidatePid).inserted else {
                continue
            }

            guard
                let candidateApp = NSRunningApplication(
                    processIdentifier: candidatePid,
                ),
                candidateApp.executableURL == executableURL
            else {
                continue
            }
            return candidatePid
    ...
    static func getOrRegister(_ nsApp: NSRunningApplication) async throws -> MacApp? {
        ...
        guard let pid = resolvedProcessIdentifier(for: nsApp) else {
            return nil
        }
        ...
        let thread = Thread {
            $axTaskLocalAppThreadToken.withValue(AxAppThreadToken(pid: pid, idForDebug: nsApp.idForDebug)) {
                ...
                let app = isGood ? MacApp(pid, nsApp, axApp, subscriptions, Thread.current) : nil
    ...
    func getFocusedWindow(_ cm: CancellationMode) async throws -> Window? {
        let windowId = try await thread?.runInLoop(cm) { [axApp, windows, pid = self.pid] job in
    ...
    private func refreshAndGetAliveWindowIds(frontmostAppBundleId: String?) async throws -> [UInt32] {
        ...
        let (alive, dead) = try await thread.runInLoop(.cancellable) { [windows, axApp, pid = self.pid] (job) -> ([UInt32], [UInt32]) in
