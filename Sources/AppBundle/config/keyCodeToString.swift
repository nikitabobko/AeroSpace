...
private func keyCodeToString(_ keyCode: UInt16) -> String? {
    ...
    var currentKeyboard = unsafe TISCopyCurrentKeyboardInputSource().takeRetainedValue()
    var rawLayoutData = unsafe TISGetInputSourceProperty(currentKeyboard, kTISPropertyUnicodeKeyLayoutData)

    if unsafe rawLayoutData == nil {
        currentKeyboard = unsafe TISCopyCurrentASCIICapableKeyboardLayoutInputSource().takeUnretainedValue()
        unsafe rawLayoutData = TISGetInputSourceProperty(currentKeyboard, kTISPropertyUnicodeKeyLayoutData)
    }

    // Get the layout
    let layoutData = unsafe unsafeBitCast(rawLayoutData, to: CFData.self)
    let layout: UnsafePointer<UCKeyboardLayout> = unsafe unsafeBitCast(CFDataGetBytePtr(layoutData), to: UnsafePointer<UCKeyboardLayout>.self)

    var deadKeyState: UInt32 = 0
    var chars = [UniChar](repeating: 0, count: 16)
    var actualLength = 0

    let error = unsafe UCKeyTranslate(
        layout,
        keyCode,
    ...
    let str = unsafe String(utf16CodeUnits: chars, count: actualLength).lowercased()
    return str
}
