...
func parseBindings(_ raw: Json, _ backtrace: ConfigBacktrace, _ errors: inout [ConfigParseError], _ mapping: [String: UInt32]) -> [String: HotkeyBinding] {
...
func parseBinding(_ raw: String, _ backtrace: ConfigBacktrace, _ mapping: [String: UInt32]) -> ParsedConfig<(CGEventFlags, UInt32)> {
    ...
    return modifiers.flatMap { modifiers -> ParsedConfig<(CGEventFlags, UInt32)> in
        key.flatMap { key -> ParsedConfig<(CGEventFlags, UInt32)> in
