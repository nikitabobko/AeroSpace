...
func parseBindings(_ raw: OrderedJson, _ backtrace: ConfigBacktrace, _ c: inout ConfigParserContext, _ mapping: [String: UInt32]) -> [HotkeyBinding] {
    ...
    // Must sort by key to ensure output is deterministic
    let sortedTable = rawTable.sorted(by: { $0.key < $1.key })

    var existingHotkeys = Set<ExpandedHotkey>()
    var result: [HotkeyBinding] = []
    for (binding, rawCommand): (String, OrderedJson) in sortedTable {
        let backtrace = backtrace + .key(binding)
        let binding = parseBinding(binding, backtrace, mapping)
            .flatMap { hotkey -> ResOrConfigParseDiagnostic<HotkeyBinding> in
                let commands = parseShellOfCommandsForConfig(rawCommand, backtrace, &c)
                return .success(HotkeyBinding(hotkey: hotkey, commands: commands))
            }
            .getOrNil(appendErrorTo: &c.errors)
        if let binding {
            if !existingHotkeys.isDisjoint(with: binding.hotkey.expanded) {
                c.errors.append(.init(backtrace, "'\(binding.hotkey.description)' Binding redeclaration"))
            }
...
func parseBinding(_ raw: String, _ backtrace: ConfigBacktrace, _ mapping: [String: UInt32]) -> ResOrConfigParseDiagnostic<Hotkey> {
    ...
    return modifiers.flatMap { modifiers -> ResOrConfigParseDiagnostic<Hotkey> in
        key.flatMap { key -> ResOrConfigParseDiagnostic<Hotkey> in
