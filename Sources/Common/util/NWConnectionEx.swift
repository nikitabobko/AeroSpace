...
extension NWConnection {
    public func write(_ msg: Codable) async -> NWError? {
        ...
        let header = withUnsafeBytes(of: Int32(mainMsg.count)) { Data($0) }
    ...
    private func read(bytes size: Int32) async -> Result<Data, NWError> {
    ...
    public func read() async -> Result<Data, NWError> {
        switch await read(bytes: 4) {
            case .success(let header):
                let count = header.withUnsafeBytes { $0.load(as: Int32.self) }
