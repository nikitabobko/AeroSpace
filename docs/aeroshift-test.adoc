...
Test if the application ID equals to Chrome: `aeroshift test %{app-bundle-id} .= com.google.Chrome`

Test if the application name contains "chrome" (case insensitive): `aeroshift test %{app-name} .~ chrome`
...
%{window-is-fullscreen}:: Boolean. Is window in fullscreen by `aeroshift fullscreen` command
