...
<subcommand-options> that take a value can be specified either as `--option value` or as `--option=value`.
For example, `aerospace list-workspaces --monitor focused --empty=no` is the same as `aerospace list-workspaces --monitor focused --empty no`.
