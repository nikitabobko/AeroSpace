...
----
+
On Intel Macs, Homebrew’s prefix is `/usr/local` (https://docs.brew.sh/Installation[docs.brew.sh]).
That's why on Intel Macs, AeroSpace additionally prepends `/usr/local/bin:/usr/local/sbin` to the `PATH`:
+
[source,toml]
----
[exec.env-vars]
