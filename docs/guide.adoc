...
By default, key bindings in the config are perceived as `qwerty` layout, and get matched based on the key code sent from your keyboard.
...
Alternatively, you can set `key-mapping.match-key-event-by` to `key-symbol` to have Aerospace interpret key bindings based on the symbol produced when pressing a key.
When set to `key-symbol`, `key-mapping.preset` and `key-mapping.key-notation-to-key-code` get ignored, and instead your selected input source gets to match keys.
Thus, a binding like `ctrl-a` only gets triggered when you press a key that produces the symbol `a`.
This allows you to use any printable characters such as ä, α, ø, א, etc. in your bindings.
