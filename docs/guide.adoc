...
:url-search-for-config: https://github.com/search?q=path%3A*aeroshift.toml&type=code
...
NOTE: Aeroshift is an unofficial fork of https://github.com/nikitabobko/AeroSpace[AeroSpace]. When upstream issues or links appear below, they are cited as upstream history or reference material.
...
Download release artifacts from this fork, not from upstream https://github.com/nikitabobko/AeroSpace[AeroSpace].

. Download the latest available zip from https://github.com/Boredphilosopher96/Aeroshift/releases[the fork releases page]
. Unpack zip
. Put unpacked `Aeroshift-v$VERSION/Aeroshift.app` into `/Applications`
. Put unpacked `Aeroshift-v$VERSION/bin/aeroshift` anywhere on `$PATH` if you want CLI access

*(Optional)* You might need to configure your shell to enable completion provided by your shell setup or by Homebrew: https://docs.brew.sh/Shell-Completion
Aeroshift ships bash, fish, and zsh completions in its release archive.

[#manual-installation]
=== Build from source

For a repo-local source setup, use `mise`:
...
Contributor and release-install notes are covered in https://github.com/Boredphilosopher96/Aeroshift/blob/main/dev-docs/development.md[Development Notes].
Upstream docs remain at https://github.com/nikitabobko/AeroSpace/tree/main/docs[AeroSpace docs].

If you see this message

----
"Aeroshift.app" can't be opened because Apple cannot check it for malicious software.
----

**Option 1** to resolve the problem

```
xattr -d com.apple.quarantine /Applications/Aeroshift.app
```

**Option 2** to resolve the problem

. navigate in Finder to `/Applications/Aeroshift.app`
. Right mouse click
. Open (yes, it's that stupid)

[#configuring-aeroshift]
== Configuring Aeroshift

[#config-location]
=== Custom config location

Aeroshift tries to find the custom config in two locations:
...
Aeroshift uses TOML format for the config.
...
If no custom config is found, Aeroshift will load the default config.
...
[source,shell]
----
cp /Applications/Aeroshift.app/Contents/Resources/default-config.toml ~/.aeroshift.toml
----
...
The initial binding mode that Aeroshift starts out with is "main".
...
Commands are the thing you use to manipulate Aeroshift and query its state.

There are two ways on how you can use commands:

. Bind keys to run Aeroshift commands. Example:
+
[source,toml]
----
[mode.main.binding]
...
* For `dvorak` and `colemak` users, Aeroshift offers preconfigured presets.
+
[source,toml]
----
[key-mapping]
...
* `exec.inherit-env-vars = true` configures whether inherit environment variables of `Aeroshift.app` or not. (The default is `true`)
...
That's why unless you override `exec` section in your config, Aeroshift falls back to the following `exec` configuration:
+
[source,toml]
----
[exec]
...
Aeroshift stores all windows and containers in a tree.
Aeroshift tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].

*Definition.* Each non-leaf node is called a "Container"

WARNING: i3 has a different terminology.
"container" in i3 is the same as "node" in Aeroshift.
...
In total, Aeroshift provides 4 possible layouts:
...
By default, Aeroshift does two types of tree normalizations:
...
Since Spaces are so hard to deal with, Aeroshift reimplements Spaces and calls them "Workspaces".
...
When you quit the Aeroshift or when the Aeroshift detects that it's about to crash, Aeroshift will place all windows back to the visible area of the screen.

Aeroshift shows the name of currently active workspace in its tray icon (top right corner), to give users a visual feedback on what workspace is currently active.

The intended workflow of using Aeroshift workspaces is to only have one macOS Space (or as many monitors you have, if `Displays have separate Spaces` is enabled) and don’t interact with macOS Spaces anymore.
...
That means, that if Aeroshift crashes badly you will still be able to manually "unhide" the windows by dragging these few pixels to the center of the screen.
...
Since Aeroshift needs a free space to hide windows in,
...
For some reason, mission control doesn't like that Aeroshift puts a lot of windows in the bottom right corner of the screen.
...
moving windows between monitors causes windows to move between different Spaces which is not correctly handled by the public APIs Aeroshift uses,
...
|👍 Public Apple API are more stable (which in turn affects Aeroshift stability)
...
[[on-window-detected]]
    ...
    if.during-aeroshift-startup = true
...
|`if.during-aeroshift-startup`
a|
* If `true` then run the callback only during Aeroshift startup.
* If `false` then run callback only *NOT* during Aeroshift startup.
...
* `if.during-aeroshift-startup = true` is useful if you want to do the initial app arrangement only on startup.

* `if.during-aeroshift-startup = false` is useful if you want to relaunch Aeroshift, but the callback has side effects that you don’t want to run on every relaunch.
...
* You can use xref:commands.adoc#list-apps[`aeroshift list-apps`] CLI command to get IDs of running applications
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger aeroshift_workspace_change FOCUSED=$AEROSHIFT_FOCUSED_WORKSPACE'
]
----

Besides the <<exec-env-vars,`exec.env-vars`>>, the process has access to the following environment variables:

* `AEROSHIFT_FOCUSED_WORKSPACE` - the workspace user switched to
* `AEROSHIFT_PREV_WORKSPACE` - the workspace user switched from

For a more elaborate example on how to integrate with Sketchybar see
xref:./goodies.adoc#show-aeroshift-workspaces-in-sketchybar[]
...
. Aeroshift takes the assigned monitor of the workspace and makes the workspace visible on the monitor
. Aeroshift focuses the workspace
...
[NOTE]
====
The only difference between Aeroshift and i3 is switching to empty workspaces.
When you switch to an empty workspace, Aeroshift puts the workspace on an assigned monitor; i3 puts the workspace on currently active monitor.

* I find that Aeroshift model works better with <<observation,_the observation_>> listed above.
* Aeroshift model is more consistent (it works the same for empty workspaces and non-empty workspaces)
====
...
Aeroshift uses the API to gently ask windows whether they are dialogs, but Aeroshift also applies some heuristics.
...
It's fine to hardcode special handling for popular applications, Aeroshift already does it.
Please see `isDialogHeuristic` function in Aeroshift sources.
...
If you can't make Aeroshift handle some keys in your config, please make sure that you don't have keys conflict with other software that might listen to global keys (e.g. skhd, Karabiner-Elements, Raycast)
