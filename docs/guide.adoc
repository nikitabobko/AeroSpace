...
. Download the latest available zip from https://github.com/saadjs/FlightDeck/releases[the FlightDeck releases page]
. Unpack zip
. Put unpacked `FlightDeck-v$VERSION/FlightDeck.app` to `/Applications`
. Put unpacked `FlightDeck-v$VERSION/bin/flightdeck` anywhere to `$PATH`
...
. Bind keys to run FlightDeck commands. Example:
+
[source,toml]
----
[mode.main.binding]
...
FlightDeck tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].
...
|👍 Public Apple API are more stable (which in turn affects FlightDeck stability)
...
a|
* If `true` then run the callback only during FlightDeck startup.
* If `false` then run callback only *NOT* during FlightDeck startup.
...
* You can use xref:commands.adoc#list-apps[`flightdeck list-apps`] CLI command to get IDs of running applications
...
xref:./goodies.adoc#show-flightdeck-workspaces-in-sketchybar[]
...
* I find that FlightDeck model works better with <<observation,_the observation_>> listed above.
