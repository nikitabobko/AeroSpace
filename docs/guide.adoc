...
:url-search-for-config: https://github.com/search?q=path%3A*airlock.toml&type=code
...
Airlock provides bash, fish and zsh completions.
...
[source,bash]
----
brew install --cask nikitabobko/tap/airlock@0.12.0
----

[#manual-installation]
=== Manual installation

. Download the latest available zip from https://github.com/nikitabobko/Airlock/releases[releases page]
. Unpack zip
. Put unpacked `Airlock-v$VERSION/Airlock.app` to `/Applications`
. Put unpacked `Airlock-v$VERSION/bin/airlock` anywhere to `$PATH`
(The step is optional. It is only needed if you want to be able to interact with Airlock from CLI)

If you see this message

----
"Airlock.app" can't be opened because Apple cannot check it for malicious software.
----

**Option 1** to resolve the problem

```
xattr -d com.apple.quarantine /Applications/Airlock.app
```

**Option 2** to resolve the problem

. navigate in Finder to `/Applications/Airlock.app`
. Right mouse click
...
Airlock tries to find the custom config in two locations:
...
Airlock uses TOML format for the config.
...
If no custom config is found, Airlock will load the default config.
...
[source,shell]
----
cp /Applications/Airlock.app/Contents/Resources/default-config.toml ~/.airlock.toml
----
...
The initial binding mode that Airlock starts out with is "main".
...
Commands are the thing you use to manipulate Airlock and query its state.

There are two ways on how you can use commands:

. Bind keys to run Airlock commands. Example:
+
[source,toml]
----
[mode.main.binding]
...
* For `dvorak` and `colemak` users, Airlock offers preconfigured presets.
+
[source,toml]
----
[key-mapping]
...
* `exec.inherit-env-vars = true` configures whether inherit environment variables of `Airlock.app` or not. (The default is `true`)
...
That's why unless you override `exec` section in your config, Airlock falls back to the following `exec` configuration:
+
[source,toml]
----
[exec]
...
Airlock stores all windows and containers in a tree.
Airlock tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].

*Definition.* Each non-leaf node is called a "Container"

WARNING: i3 has a different terminology.
"container" in i3 is the same as "node" in Airlock.
...
In total, Airlock provides 4 possible layouts:
...
By default, Airlock does two types of tree normalizations:
...
Since Spaces are so hard to deal with, Airlock reimplements Spaces and calls them "Workspaces".
...
When you quit the Airlock or when the Airlock detects that it's about to crash, Airlock will place all windows back to the visible area of the screen.

Airlock shows the name of currently active workspace in its tray icon (top right corner), to give users a visual feedback on what workspace is currently active.

The intended workflow of using Airlock workspaces is to only have one macOS Space (or as many monitors you have, if `Displays have separate Spaces` is enabled) and don’t interact with macOS Spaces anymore.
...
That means, that if Airlock crashes badly you will still be able to manually "unhide" the windows by dragging these few pixels to the center of the screen.
...
Since Airlock needs a free space to hide windows in,
...
For some reason, mission control doesn't like that Airlock puts a lot of windows in the bottom right corner of the screen.
...
* Wrong window may receive focus in multi-monitor setup: https://github.com/nikitabobko/Airlock/issues/101[#101] (Bug in Apple API)
* Wrong borderless Alacritty window may receive focus in *single monitor* setup: https://github.com/nikitabobko/Airlock/issues/247[#247] (Bug in Apple API)
* Performance issues: https://github.com/nikitabobko/Airlock/issues/333[#333]
* macOS randomly switches focus back: https://github.com/nikitabobko/Airlock/issues/289[#289]

When `Displays have separate Spaces` is enabled,
moving windows between monitors causes windows to move between different Spaces which is not correctly handled by the public APIs Airlock uses,
...
|👍 Public Apple API are more stable (which in turn affects Airlock stability)
...
[[on-window-detected]]
    ...
    if.during-airlock-startup = true
...
|`if.during-airlock-startup`
a|
* If `true` then run the callback only during Airlock startup.
* If `false` then run callback only *NOT* during Airlock startup.
...
* `if.during-airlock-startup = true` is useful if you want to do the initial app arrangement only on startup.

* `if.during-airlock-startup = false` is useful if you want to relaunch Airlock, but the callback has side effects that you don’t want to run on every relaunch.
...
* You can use xref:commands.adoc#list-apps[`airlock list-apps`] CLI command to get IDs of running applications
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger airlock_workspace_change FOCUSED=$AIRLOCK_FOCUSED_WORKSPACE'
]
----

Besides the <<exec-env-vars,`exec.env-vars`>>, the process has access to the following environment variables:

* `AIRLOCK_FOCUSED_WORKSPACE` - the workspace user switched to
* `AIRLOCK_PREV_WORKSPACE` - the workspace user switched from

For a more elaborate example on how to integrate with Sketchybar see
xref:./goodies.adoc#show-airlock-workspaces-in-sketchybar[]
...
. Airlock takes the assigned monitor of the workspace and makes the workspace visible on the monitor
. Airlock focuses the workspace
...
[NOTE]
====
The only difference between Airlock and i3 is switching to empty workspaces.
When you switch to an empty workspace, Airlock puts the workspace on an assigned monitor; i3 puts the workspace on currently active monitor.

* I find that Airlock model works better with <<observation,_the observation_>> listed above.
* Airlock model is more consistent (it works the same for empty workspaces and non-empty workspaces)
====
...
Airlock uses the API to gently ask windows whether they are dialogs, but Airlock also applies some heuristics.
...
It's fine to hardcode special handling for popular applications, Airlock already does it.
Please see `isDialogHeuristic` function in Airlock sources.
...
If you can't make Airlock handle some keys in your config, please make sure that you don't have keys conflict with other software that might listen to global keys (e.g. skhd, Karabiner-Elements, Raycast)
