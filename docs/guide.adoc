...
. Download the latest available zip from https://github.com/AdrianLSY/AeroSpace/releases[releases page]
. Unpack zip
...
- `h_dwindle` horizontal dwindle (Hyprland-style binary-tree split, this fork only)
- `v_dwindle` vertical dwindle (Hyprland-style binary-tree split, this fork only)
...
NOTE: Dwindle is a fork-specific feature. It is *not* available in upstream `nikitabobko/AeroSpace`. See https://github.com/nikitabobko/AeroSpace/issues/260[upstream issue #260] for the upstream tracking issue on dynamic layouts.

The `dwindle` layout renders identically to `tiles` but uses a different
*insertion* algorithm: a new window wraps the focused window in a new dwindle
container of opposite orientation, then joins it as a sibling. The result is a
binary tree with alternating-orientation splits, matching the dwindle layout
in Hyprland and other dynamic tilers.

You can opt in via `default-root-container-layout = 'dwindle'` (sets the
default for new workspaces) or by running `aerospace layout dwindle` to flip a
specific container.

Compared to `tiles`, where new windows append as siblings of the focused
window's container:

----
# tiles, after opening 4 windows in sequence
h_tiles
├── window 1
├── window 2
├── window 3
...
dwindle wraps the focused window each time:

----
# dwindle, after opening 4 windows in sequence (each window splits the previous focus)
h_dwindle
├── window 1
└── v_dwindle
    ├── window 2
...
Compared to `accordion` (windows stacked, only the focused one visible),
dwindle keeps every window visible, just allocated to progressively smaller
slices of the screen.

Behaviour is tunable via the <<dwindle-config,`[dwindle]` configuration
section>>.

*Hyprland features intentionally not implemented:* `smart_resizing` (conflicts
with AeroSpace's tree-walk resize semantics), `permanent_direction_override`
(niche), `pseudotile` (overlaps with `aerospace layout floating`),
`special_scale_factor` (no special-workspace concept). See the
https://wiki.hyprland.org/Configuring/Dwindle-Layout/[Hyprland dwindle docs]
for context.

[#dwindle-config]
==== `[dwindle]` configuration

All keys are optional. Defaults match "vanilla Hyprland dwindle behaviour, but
conservative" — opt into the more aggressive options explicitly.

[source,toml]
----
[dwindle]
    force-split             = 'auto'    # default: 'auto'. one of: auto|first|second
    smart-split             = false     # default: false
    preserve-split          = false     # default: false
    default-split-ratio     = 0.5       # default: 0.5. Must be in (0.0, 1.0)
    split-width-multiplier  = 1.0       # default: 1.0. Must be > 0
    no-gaps-when-only       = false     # default: false
    use-active-for-splits   = true      # default: true
----
...
|`force-split`
|Which side of the focused window the new window joins. `auto` matches Hyprland's default (right/bottom). `first` puts the new window on the left/top. `second` is the explicit form of `auto`.

|`smart-split`
|If `true`, cursor position inside the focused window decides split orientation and side, overriding `force-split`. Closest edge to the cursor wins: cursor near the left edge → vertical split with new window on the left, near the bottom edge → horizontal split with new window below, etc. Cursor outside the focused window's rect falls back to `force-split` and aspect-ratio default.

|`preserve-split`
|If `true`, dwindle-created split containers survive when reduced to a single child (instead of being collapsed by `enable-normalization-flatten-containers`). Closing one side of a split keeps the split direction so the next inserted window picks up the same orientation. Zero-child cleanup is unaffected — fully-emptied splits are still removed.

|`default-split-ratio`
|Initial weight ratio between the focused window and the new window, in the open interval `(0.0, 1.0)`. `0.5` produces an even split; `0.7` makes the new window 70% of the slot.

|`split-width-multiplier`
|Multiplier applied on top of `default-split-ratio` to compute the new window's weight. Effective ratio = `default-split-ratio * split-width-multiplier`, clamped to `[0.05, 0.95]`. Must be `> 0`.

|`no-gaps-when-only`
|If `true`, a workspace that contains exactly one window drops outer gaps and uses the monitor's full visible rect. Adding a second window restores gaps. Scoped to dwindle-rooted workspaces.

|`use-active-for-splits`
|Default `true`: splits target the focused (most-recent) window. If `false`, splits target the window under the cursor at insertion time, falling back to the focused window when the cursor is over no window.
|===
...
AeroSpace bundles a port of https://github.com/sbmpost/AutoRaise[AutoRaise], so focus can follow the mouse cursor into any window on the currently-focused workspace.

* Disabled by default. Enable it with `[auto-raise] enabled = true`.
* Only windows belonging to the currently-focused workspace are raised.
Hovering a window that lives on a non-focused workspace (for example on another monitor) never switches workspace, matching i3 behavior.
* Raises go through the same focus machinery as xref:./commands.adoc#focus[`focus`] command — the tree's most-recent-child pointer, monitor active-workspace state, and xref:#on-focus-changed-callbacks[`on-focus-changed`] callbacks stay consistent.

[#auto-raise-on-focus-changed-gotcha]
[IMPORTANT]
====
`on-focus-changed` fires on every hover-raise, because a hover-raise is a focus change.
If you have an expensive `on-focus-changed` callback, that command will run each time the cursor crosses a window boundary.
Keep the callback cheap, or disable auto-raise for the workspaces / monitors where you don't want it.
====
...
[auto-raise]
    ...
    poll-millis             = 8              # default: 8 (≈120 Hz throttle), min: 1
    ignore-space-changed    = false          # default: false
    invert-disable-key      = false          # default: false
    invert-ignore-apps      = false          # default: false
    ignore-apps             = []             # default: []. Application names. Exact match
    ignore-titles           = []             # default: []. ICU regex applied to window title
    stay-focused-bundle-ids = []             # default: []. Application bundle IDs
    disable-key             = "control"      # default: "control". one of: control|option|disabled
----
...
|`enabled`
|Master switch. `false` keeps the event tap uninstalled. Runtime `enable-auto-raise` / `disable-auto-raise` commands override this — once you disable at runtime, reloading the config does not silently re-enable.

|`poll-millis`
|Minimum milliseconds between mouse-moved evaluations. Lower = more responsive, higher = less CPU. Default 8 ≈ 120 Hz. Must be ≥ 1.

|`ignore-space-changed`
|If `true`, switching macOS Spaces does not re-evaluate the window under the cursor. Useful if you toggle Spaces via gestures and don't want focus to jump.

|`invert-disable-key`
|If `true`, auto-raise only operates while `disable-key` is held (opt-in model). Default `false` means holding the modifier suppresses raises (opt-out model).

|`invert-ignore-apps`
|If `true`, `ignore-apps` becomes an allow-list — only listed apps participate in hover-raise.

|`ignore-apps`
|Application names (as shown in the menu bar — e.g. `"Finder"`, `"Safari"`) that are ignored by hover-raise. Exact match, case-sensitive.

|`ignore-titles`
|Window-title patterns that are ignored. Entries are ICU regular expressions (same engine `NSRegularExpression` uses). Invalid regex fails config parsing.

|`stay-focused-bundle-ids`
|Application bundle IDs (e.g. `"com.apple.SecurityAgent"`) that refuse to lose focus to hover-raise. Once focused, the cursor can move freely over other windows without losing focus until you focus something else via keyboard / click / command.

|`disable-key`
|Modifier that temporarily disables hover-raise while held. One of `"control"`, `"option"`, or `"disabled"` (no disable modifier). Apple's `cmd` and `shift` are not available here because they're already overloaded by macOS.
|===

[#auto-raise-commands]
=== Runtime toggle

* xref:./commands.adoc#enable-auto-raise[`enable-auto-raise`] — start hover-raise using the loaded `[auto-raise]` config. Clears any prior runtime-disable. Note: this is a toggle, not a reload — if the bridge is already running, the command does not re-read `[auto-raise]` from disk. Use xref:./commands.adoc#reload-config[`reload-config`] to apply TOML edits.
* xref:./commands.adoc#disable-auto-raise[`disable-auto-raise`] — stop hover-raise and set a sticky flag. Subsequent config reloads (via xref:./commands.adoc#reload-config[`reload-config`] or the file watcher) will NOT re-enable until `enable-auto-raise` is run again.
* Both commands are idempotent by default. Pass `--fail-if-noop` to get a non-zero exit when the command doesn't change state — useful for scripts.
* The master xref:./commands.adoc#enable[`enable off`] command also pauses hover-raise — mouse movement stops driving focus while window management is disabled. `enable on` resumes hover-raise at its prior running state. A sticky runtime-disable from `disable-auto-raise` survives the off/on cycle.

Example keybinding:
[source,toml]
----
[mode.main.binding]
    alt-shift-slash = 'enable-auto-raise'
    alt-slash       = 'disable-auto-raise'
----

[#auto-raise-mouse-follows-focus]
=== Hover-raise vs. mouse-follows-focus

These are two different behaviors that are easy to confuse:

* **Hover-raise** (this section) — moving the mouse changes focus.
* **Mouse-follows-focus** — changing focus (via a keybinding, say xref:./commands.adoc#focus[`focus left`]) moves the mouse cursor onto the newly-focused window. Configured with xref:./commands.adoc#move-mouse[`move-mouse`] in an xref:#on-focus-changed-callbacks[`on-focus-changed`] callback.

Both can be enabled simultaneously. When they are, the `move-mouse` fired from `on-focus-changed` does not re-trigger hover-raise — it resettles the cursor inside the already-focused window.

[#auto-raise-license]
[NOTE]
====
Linking AutoRaise into AeroSpace makes the combined binary GPL-2.0-or-later. Source files outside `Sources/AutoRaiseCore/` keep their original license (MIT for most of the project). See `LICENSE.txt` and `LICENSE-GPL` at the repo root.
====
