...
. Navigate to `/Applications/AeroSpace.app` in Finder
...
A list of available commands is provided in the xref:commands.adoc[commands documentation].
...
Disabling "Displays have separate Spaces" will grant you:
...
Please post your use cases to https://github.com/nikitabobko/AeroSpace/issues/20[this GitHub issue] if you want additional commands to be supported.
...
** Refer to our xref:./goodies.adoc#popular-apps-ids[list of Apple application IDs]
...
macOS provides an accessibility API for identifying dialog windows, but many apps either use it incorrectly or not at all. Even some Apple apps (like Finder’s "Copy" progress window) fail to mark dialogs properly. AeroSpace uses the API when available but also applies additional heuristics to identify dialogs.

=== Heuristic rules

* Windows without a fullscreen button (NB! fullscreen button and maximize button are different buttons) are treated as dialogs, excluding terminal apps (e.g. WezTerm, Alacritty, iTerm2)
