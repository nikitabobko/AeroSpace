...
The first two windows follow the container's orientation (if only one tiled window is left in the workspace, the
root container is reset to its default orientation). When the most recently
focused window belongs to a `tiles` container with two or more children, a new
window splits that tile. If a floating window is focused, the most recently focused tile
is split.

The tile is split along its longer side: a tile that is wider than tall is split side by side, a tile that is taller
than wide is stacked. For example, on a regular monitor two side-by-side windows become a full-height window next to
two vertically stacked windows, while on an ultrawide monitor they become three columns.
The new window goes below the previous window for a vertical split, or to its
right for a horizontal split. This does not impose a permanent two-column limit.
...
positions still take precedence.

The following actions place the window in the same way as a newly opened window:

* Moving a tiled window to another workspace or monitor (`move-node-to-workspace`, `move-node-to-monitor`)
* Making a floating window tiling (`layout tiling`)
* Exiting macOS native fullscreen, unminimizing, or unhiding the app

Moving windows inside a workspace (`move`) doesn't trigger automatic splitting.
Changing the setting does not rearrange existing windows.

Dragging a tiling window with mouse is also different when automatic tiling is enabled. The tree is changed once the
window is dropped, instead of swapping the windows while dragging:

* Dropping the window onto the center of another tile swaps the windows
* Dropping the window onto a side of another tile (30% from the edge) splits that tile. The dropped window takes the half
