...
* GUI apps on macOS don’t have Homebrew’s prefix in their `PATH` by default ([source](https://docs.brew.sh/FAQ#my-mac-apps-dont-find-homebrew-utilities)).
That's why unless you override `exec` section in the config, AeroSpace fallbacks to the following `exec` configuration:
+
[source,toml]
----
[exec]
