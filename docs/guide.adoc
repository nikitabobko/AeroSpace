...
If you want to minimize the visibility of hidden windows, it's recommended to place Dock in the bottom (and additionally turn off automatic hiding)
====
...
If you find that some windows are not handled properly please https://github.com/nikitabobko/AeroSpace/issues/new[report an issue] (don't forget to attach xref:./commands.adoc#debug-windows[debug-windows output]).
