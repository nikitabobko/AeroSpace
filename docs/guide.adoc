...
Moving a tiled window to another workspace or monitor
(`move-node-to-workspace`, `move-node-to-monitor`) splits the most recently focused tile of the target workspace in the
same way. Moving windows inside a workspace (`move`), moving them with mouse, and toggling an existing window from
floating to tiling do not trigger automatic splitting. Changing the setting
