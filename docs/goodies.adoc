...
NOTE: AeroShift is an unofficial fork of https://github.com/nikitabobko/AeroSpace[AeroSpace]. Some third-party integrations below were originally written for https://github.com/nikitabobko/AeroSpace[AeroSpace] and may need small command-name adjustments for `aeroshift`.

Do you have a cool automatization, AeroShift integration, or workflow?
...
You can also use `after-startup-command` to start JankyBorders together with AeroShift

[source,toml]
----
after-startup-command = [
    # JankyBorders has a built-in detection of already running process,
    # so it won't be run twice on AeroShift restart
...
There is a third-party Raycast extension for https://github.com/nikitabobko/AeroSpace[AeroSpace]: https://www.raycast.com/limonkufu/aerospace

It assumes upstream naming, so review and adapt its commands before using it with AeroShift.
...
[source,bash]
----
aeroshift workspace "$(aeroshift list-workspaces --monitor mouse --visible)" && aeroshift workspace next

aeroshift workspace "$(aeroshift list-workspaces --monitor mouse --visible)" && aeroshift workspace prev
----
...
Beware that you might need to specify the full path to the `aeroshift` executable: https://community.folivora.ai/t/how-to-execute-terminal-command-to-switch-workspaces-with-aerospace/35914
...
[#show-aeroshift-workspaces-in-sketchybar]
== Show AeroShift workspaces in Sketchybar

You can integrate AeroShift workspace indicators with link:https://github.com/FelixKratz/SketchyBar[Sketchybar].
Use these snippets as a starting point.

.~/.aeroshift.toml
[source,toml]
----
# Run Sketchybar together with AeroShift
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger aeroshift_workspace_change FOCUSED_WORKSPACE=$AEROSHIFT_FOCUSED_WORKSPACE'
]
----
...
[source,bash]
----
sketchybar --add event aeroshift_workspace_change

for sid in $(aeroshift list-workspaces --all); do
    sketchybar --add item space.$sid left \
        --subscribe space.$sid aeroshift_workspace_change \
        --set space.$sid \
        ...
        label="$sid" \
        click_script="aeroshift workspace $sid" \
        script="$CONFIG_DIR/plugins/aeroshift.sh $sid"
done
----

.~/.config/sketchybar/plugins/aeroshift.sh
[source,bash]
----
#!/usr/bin/env bash

# make sure it's executable with:
# chmod +x ~/.config/sketchybar/plugins/aeroshift.sh
...
[#show-aeroshift-workspaces-in-simple-bar]
== Show AeroShift workspaces in simple-bar

link:https://github.com/Jean-Tinland/simple-bar[simple-bar] can be used to display AeroShift workspaces.

In order to sync simple-bar with AeroShift, add these lines to your AeroShift config file:
...
Opening *a new window* of a program that can support multiple windows (such as Safari or Terminal.app) can be accomplished with an AppleScript inlined in `aeroshift.toml` as follows:
