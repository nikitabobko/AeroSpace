...
If you are using _simple-bar_ *without simple-bar-server*, add these lines to your AeroSpace config file:
...
This approach may feel a bit slow while refreshing simple-bar as it relies on the applescript implementation of Übersicht. If you want to obtain better performances, you can install simple-bar-server and use the instructions below.

Feel free to open an issue in link:https://github.com/Jean-Tinland/simple-bar/issues[simple-bar repo] if you encounter any issue.

If you are using link:https://github.com/Jean-Tinland/simple-bar-server[simple-bar-server], you'll need to add these lines instead:

.~/.aerospace.toml
[source,toml]
----
# Notify simple-bar-server to trigger a simple-bar refresh on window focus change
on-focus-changed = [
  "exec-and-forget curl http://localhost:7776/aerospace/spaces/refresh",
]

# Notify simple-bar-server to trigger a simple-bar refresh
# You'll speed up the process by passing the "$AEROSPACE_FOCUSED_WORKSPACE" in a "space" query param
exec-on-workspace-change = [
  '/bin/zsh',
  '-c',
...
Feel free to open an issue in link:https://github.com/Jean-Tinland/simple-bar-server/issues[simple-bar-server repo] if you encounter any issue.
