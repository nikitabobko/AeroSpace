...
Do you have a cool automatization, Airlock integration, or workflow?
...
[#raycast-extension]
== Airlock Raycast extension

There is a third party Raycast extension for Airlock. https://www.raycast.com/limonkufu/airlock
...
* https://github.com/acsandmann/airlock-swipe
* link:https://folivora.ai/[BetterTouchTool] can assign arbitrary commands to trackpad gestures.
Beware that you might need to specify full path to airlock executable https://community.folivora.ai/t/how-to-execute-terminal-command-to-switch-workspaces-with-airlock/35914
* https://github.com/MediosZ/SwipeAirlock
...
[#show-airlock-workspaces-in-sketchybar]
== Show Airlock workspaces in Sketchybar

You can integrate Airlock workspace indicators with link:https://github.com/FelixKratz/SketchyBar[Sketchybar].
Use these snippets as a starting point.

.~/.airlock.toml
[source,toml]
----
# Run Sketchybar together with Airlock
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger airlock_workspace_change FOCUSED_WORKSPACE=$AIRLOCK_FOCUSED_WORKSPACE'
]
----
...
[source,bash]
----
sketchybar --add event airlock_workspace_change

for sid in $(airlock list-workspaces --all); do
    sketchybar --add item space.$sid left \
        --subscribe space.$sid airlock_workspace_change \
        --set space.$sid \
        ...
        label="$sid" \
        click_script="airlock workspace $sid" \
        script="$CONFIG_DIR/plugins/airlock.sh $sid"
done
----

.~/.config/sketchybar/plugins/airlock.sh
[source,bash]
----
#!/usr/bin/env bash

# make sure it's executable with:
# chmod +x ~/.config/sketchybar/plugins/airlock.sh
...
[#show-airlock-workspaces-in-simple-bar]
== Show Airlock workspaces in simple-bar

link:https://github.com/Jean-Tinland/simple-bar[simple-bar] can be used to display Airlock workspaces.

In order to sync simple-bar with Airlock, add these lines to your Airlock config file:
