...
Options that take a value can be specified either as `--option value` or as `--option=value`.
For example, `list-workspaces --monitor focused --empty=no` is the same as `list-workspaces --monitor focused --empty no`.
