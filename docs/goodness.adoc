...
Opening a new window of a program that can supports multiple windows (such as Safari or Apple Terminal) can be accomplished with an AppleScript inlined in `aerospace.toml` as follows:

[source,toml]
----
ctrl-g = '''exec-and-forget osascript <<EOF
tell application "MyApp"
    -- often an extra command is needed here, see comment below!
    activate
end tell
EOF'''
----
...
- Safari.app needs `make new document at end of documents` before the `activate` line
- Terminal.app needs `do script` before the `activate` line
