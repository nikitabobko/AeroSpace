...
[#safari-new-window-command]
== Open a new Safari window with a command

Invoking Safari with a command the obvious way (`exec-and-forget open -a Safari`) results in an outcome that is probably not the intended one, namely that any workspace already containing an instance of Safari is brought in focus.
In case one wants instead to open a new Safari window in the current workspace, this can be accomplished as follows: 

1. Save this script somewhere (for example `~/bin/safari-new-window.scpt`).
...
2. Add this or a similar keybinding to `~/.aerospace.toml`:

`ctrl-g = ['exec-and-forget osascript ~/bin/safari-new-window.scpt', 'mode main']`

|===

|===

[#terminal-new-window-command]
== Open a new Terminal window with a command

Similarly to Safari, invoking the Terminal app with a command the obvious way (`exec-and-forget open -a Terminal`) results in an outcome that is probably not the intended one, namely that any workspace already containing an instance of the Terminal app is brought in focus.
In case one wants instead to open a new Terminal window in the current workspace, this can be accomplished as follows: 

1. Save this script somewhere (for example `~/bin/terminal-new-window.scpt`).
...
`ctrl-g = ['exec-and-forget osascript ~/bin/terminal-new-window.scpt', 'mode main']`
